...
| git switch -c <name> | 新建分支并切换（Git 2.23+ 推荐，替代 checkout） |
| git switch <name> | 切换分支 |
| git merge <branch> | 将目标分支合并到当前分支 |
| git branch -d <name> | 删除本地分支（-D 强制删） |
...
| git remote add origin <url> | 关联本地仓库到远程 |
...
| git restore <file> | 丢弃工作区修改（慎用，不可恢复） |
| git restore --staged <file> | 撤回 add（从暂存区移除，保留修改） |
